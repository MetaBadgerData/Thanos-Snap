...
hardy_weinberg <- function(p, pop) {
  ...
  if (total != pop) {
    diff <- pop - total
    # Distribute the difference to the largest genotype
...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      actionButton("snapButton", "SNAP!"),
      actionButton("resetButton", "RESET"),  # Added reset button
...
server <- function(input, output, session) {
  ...
  # Timer for continuous growth - FIXED VERSION
  observe({
    if (growing()) {
      ...
      isolate({
        ...
        if (!is.null(current_data) && length(current_data) > 0) {
          ...
          if (phase() == "Pre-snap") {
            # Get the last generation from pre-snap data
  ...
  output$genotypePlot <- renderPlot({
    ...
    if (length(generations) > 1) {
      ...
      if (length(current_data) > 1) {
        ...
        abline(v = snap_gen, lty = 2, col = "red", lwd = 2)
        text(snap_gen, max(AA_counts + Aa_counts + aa_counts) * 0.95, 
             "SNAP!", col = "red", pos = 4, cex = 1.2)
      }
    }
